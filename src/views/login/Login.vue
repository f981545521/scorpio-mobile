    ...
    <div @click="changeImage(3)"
      ...
      <div class="auth-form">
        ...
        <van-tabs animated
          ...
          <van-tab :title="$t('login.title')">
            ...
            <van-cell-group v-show="!isShowSMSLogin">
              <van-field :error-message="phoneNumberRight?'':$t('login.phoneNumberNotCorrect')"
              ...
              <van-field :placeholder="$t('login.varify')"
                         center
                         clearable
                         maxlength="4"
                         v-model="imgCaptcha">
                <img @click.prevent="getCaptcha"
                     alt="captcha"
            ...
            <van-cell-group v-show="isShowSMSLogin">
              <van-field :error-message="phoneNumberRight?'':$('login.phoneNumberNotCorrect')"
                         :label="$t('login.phoneNumber')"
                         :placeholder="$t('login.phoneInput')"
                         clearable
                         maxlength="11"
                         required
                         v-model="login_phone"/>
              <van-field :label="$t('login.varify')"
                         ...
                         required
                         v-model="smsCaptcha">
                <van-button :disabled="captchaDisable"
                            @click="sendVerifyCode"
                            size="small"
                            slot="button"
                            type="primary"
                            v-if="!countDown">{{$t('login.sendVerify')}}
                </van-button>
                <van-button disabled=""
                            size="small"
                            slot="button"
                            type="primary"
                            v-else
                            v-model="smsCaptcha">{{$t('login.hasSend')}}{{countDown}}s
                </van-button>
              </van-field>
            </van-cell-group>
            <van-button @click='login'
                        ...
                        type="info">{{$t('login.login')}}
            </van-button>
            <div @click="switchLogin"
                 class="switchLogin">{{this.isShowSMSLogin?$t('login.phoneVerify'):$t('login.smsMessage')}}
            </div>
          </van-tab>
          <!-- 注册 -->
          <van-tab :title="$t('login.resgin')">
            <van-cell-group>
              <van-field :label="$t('login.phoneNumber')"
                         :placeholder="$t('login.phoneInput')"
                         clearable
                         maxlength="11"
                         required
        ...
        <p class="agreement"> {{$t('login.tipTile')}}<br>{{$t('login.tipContent')}}<a @click.stop="agreement(0)"
                                                                                      class="agreement-box">{{$t('login.tip')}}</a>、<a
